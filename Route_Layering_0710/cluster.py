...
def cluster(G,distance_margin,ots_margin,osnr_margin,ex):
    ...
    for start_node in G.nodes():
                ...
                for n2 in G.neighbors(current_node):
                    if not End:
                        if G.nodes[n2]['relay'] == False:
                            #非中继-非中继'
                            End=False
                            new_subgraph,visited,queue,End2 = sub_extend(G,visited,queue,new_subgraph,current_node,n2,distance_margin,ots_margin,osnr_margin)
                            if End2:
                                break
                        else:
                            #非中继-中继
        ...
        if all(not set(new_subgraph.edges()).intersection(set(sg.edges())) for sg in subgraphs) and not nx.is_empty(new_subgraph):
            print(num)
            num+=1
    ...
    for d in new_subgraph.nodes():
        ...
        if  nx.has_path(new,n2,d):
            ...
            for i in range(len(path)-1):
                path_distance += max(new.get_edge_data(path[i], path[i+1])[j]['distance'] for j in range(len(new.get_edge_data(path[i], path[i+1]))))
                path_ots += max(new.get_edge_data(path[i], path[i+1])[j]['ots'] for j in range(len(new.get_edge_data(path[i], path[i+1]))))
                path_osnr += max(new.get_edge_data(path[i], path[i+1])[j]['osnr'] for j in range(len(new.get_edge_data(path[i], path[i+1]))))
